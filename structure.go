...
func getUsers(apiKey string) (map[string]User, error) {
	body, err := flowpushGET(apiKey, flowdockAPIURL+"/users")
	if err != nil {
		return nil, err
	}
...
func getOrganizations(apiKey string) ([]Organization, error) {
	body, err := flowpushGET(apiKey, flowdockAPIURL+"/organizations")
	if err != nil {
		return nil, err
	}
...
func getFlows(apiKey string) ([]Flow, error) {
	body, err := flowpushGET(apiKey, flowdockAPIURL+"/flows")
	if err != nil {
		return nil, err
	}
